...
//************* Start Initial Sequence **********//		
	...
	lcddev.width=320;
	lcddev.height=480;			
	LCD_WriteReg(0x36,(1<<3)|(0<<5)|(0<<6)|(0<<7));//BGR==1,MY==1,MX==0,MV==1
#else//竖屏
	lcddev.dir=0;//竖屏				 	 		
	lcddev.width=320;
	lcddev.height=480;	
...
//lsw添加 float函数
uint8_t * ftoa(float db, uint8_t *str)
{
  uint8_t *istr = str;//备份首地址
  if (db < 0)
  {
    *str = '-';//是负数
    db *= -1;///负数转化为正数
  ...
  *str = '.';//处理小数点
  ...
  float xiaoshu = db - (int)db;//取出小数
  int  xiaoshuwei = 0;
  //0.123   1.23  1   12.3 12  123 123
  //取出小数位
  //ixiaoshu != (int)ixiaoshu精确描述
  
  for (float ixiaoshu = xiaoshu; ixiaoshu != (int)ixiaoshu; ixiaoshu *= 10)
  {
...
void LCD_ShowFloat(u16 x,u16 y,u16 width,u16 height,u8 size,float num)
{
  uint8_t ch[15];
  ftoa(num,ch);
  LCD_ShowString(x,y,width,height,size,ch);
}
